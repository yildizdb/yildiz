...
const EdgeLifetime = require("./db/EdgeLifetime.js");
...
class Yildiz {
    ...
    async init(/* no options used anymore */){
        ...
        this.lookupCache = new LookupCache(this.config, this.metrics, this);
        this.fetchJob = new FetchJob(this.config, this, this.metrics);
        this.edgeLifetime = new EdgeLifetime(this.config, this, this.metrics);
    ...
    async _generate(){
        ...
        if (!columnFamilyNodeExists || !columnFamilyNodeExists[0]) {
            await columnFamilyNode.create(gcRule);
        }
        ...
        if (!columnFamilyTTLExists || !columnFamilyTTLExists[0]) {
            await columnFamilyTTL.create(gcRule);
        }
        ...
        if (!columnFamilyMetadataExists || !columnFamilyMetadataExists[0]) {
            await columnFamilyMetadata.create(gcRule);
        }
        ...
        if (!columnFamilyPopnodeExists || !columnFamilyPopnodeExists[0]) {
            await columnFamilyPopnode.create(gcRule);
        }
        ...
        if (!columnFamilyCacheExists || !columnFamilyCacheExists[0]) {
            await columnFamilyCache.create(gcRule);
        }
    ...
    _runJobs(){
        ...
        if(this.config.edgeLifetime && typeof this.config.edgeLifetime === "object"){
            this.edgeLifetime.init();
        } else {
            debug("edgeLifetime job configuration missing.");
        }
    }
    ...
    async close(){
        ...
        if(this.lookupCache){
            await this.lookupCache.close();
        }

        if (this.edgeLifetime) {
            this.edgeLifetime.close();
        }
