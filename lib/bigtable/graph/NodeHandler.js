...
class NodeHandler {
    ...
    async edgeExistsId(firstId, secondId, relation = 0) {
        ...
        const identifier = strToInt(firstId + "_" + secondId + "_" + relationInt) + "";

        const cacheKey = `eei:${identifier}`;
        ...
        return new Promise((resolve, reject) => {
            ...
            .on("end", async () => {
                ...
                const result = await this._getRow(idFound, this.edgeTable, this.columnFamilyEdge.familyName);
    ...
    async increaseEdgeDepthById(first, second, relation = 0) {
        ...
        try {
            await row.save(`${this.columnFamilyEdge.familyName}:depth`, parseInt(edge.depth) + 1 + "");
        }
        catch (err) {
        ...
        const result = edge;
        edge.depth = parseInt(edge.depth) + 1;

        const cacheKey = `eei:${edge.id}`;
        await this.yildiz.cache.setEdge(cacheKey, result);
    ...
    async increaseEdgeDepthByEdgeId(edgeId) {
    ...
    async decreaseEdgeDepthById(first, second, relation = 0) {
        ...
        const cacheKey = `eei:${edge.identifier}`;
        await this.yildiz.cache.setEdge(cacheKey, result);
