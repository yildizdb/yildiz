...
class NodeHandlerSingle {
    ...
    async createEdgeWithId(fnId, snId, relation = 0, attributes = {}, _extend = {}, ttld = false, depthMode = false, isPopularRightNode = false, edgeTime = Date.now()) {
        ...
        if (this.dbConfig.rightNodeEdge) {
           ...
           //TODO: if popularRightEdge mode treat differently (currently just dont write it to db)
            ...
            // Save popularnode edge data in separate table
                ...
                const key = `${fnId}#${snId}#${relation}`;
                const row = this.popnodeTable.row(key);
                const column = depthMode ? "depth" : "data";
                
                const qualifierData = `${this.columnFamilyPopnode.familyName}:${column}`;
                requests.push(depthMode ? row.increment(qualifierData) : row.save(qualifierData,val));
                
                const qualifierTime = `${this.columnFamilyPopnode.familyName}:edgeTime`;
                requests.push(row.save(qualifierTime, edgeTime));
    ...
    async increaseEdgeDepthById(fnId, snId, isPopularRightNode = false) {
        ...
        if (this.dbConfig.rightNodeEdge) {
            ...
            if (!isPopularRightNode) {
                ...
                const key = `${fnId}#${snId}`;
                const row = this.popnodeTable.row(key);
                const qualifier = `${this.columnFamilyPopnode.familyName}:depth`;
                requests.push(row.increment(qualifier));        
            }
        }
    ...
    async getEdgeTime(first, second, relation = 0) {
        ...
        const identifier = `${first}#${second}#${relation}`;

        const edge = await this._getRow(identifier, this.popnodeTable, this.columnFamilyPopnode.familyName);
    ...
    async getNodeByIdentifier(identifier) {
        ...
        const node = await this._getRow(identifier, this.nodeTable, this.columnFamilyNode.familyName);
    ...
    async getCacheByIdentifier(identifier) {
        ...
        const cache = await this._getRow(identifier, this.cacheTable, this.columnFamilyCache.familyName);
    ...
    async createCache(cache) {
        ...
        const cfName = this.columnFamilyCache.familyName;
        const qualifier = `${cfName}:value`;
        
        const rowTTL = this.ttlTable.row(`${cache.identifier}_caches`);
        const cfNameTTL = this.columnFamilyTTL.familyName;
        const qualifierTTL = `${cfNameTTL}:value`;

        await Promise.all([
            row.save(qualifier, JSON.stringify(cache)),
            rowTTL.save(qualifierTTL, "1")
        ]);
