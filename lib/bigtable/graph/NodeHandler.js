...
class NodeHandler {
    ...
    async _updateEdgeOnNode(nodeId, edgeId) {
        ...
        try {
            ...
            await this.createNode(null, null, null, null, null, edgeId);
        }
    ...
    async getEdgeByIdentifier(identifier) {
        ...
        let result = await this._getRow(identifier, this.edgeTable, this.columnFamilyEdge.familyName);
        return result;
    }

    async getEdgeCount() {
        return this.metadata.getCount("edges");
    }
    ...
    async createNode(identifier = generateId(), properties = {}, _extend = {}, ttld = false, id, edge) {
        ...
        const result = Object.assign({identifier: key, id: key},{ttld},{data});
    ...
    async getNodeByIdentifier(identifier) {
        ...
        let result = await this._getRow(identifier, this.nodeTable, this.columnFamilyNode.familyName);
        return new Node(null, result);
    }

    async getNodeCount() {
        return this.metadata.getCount("nodes");
    }
