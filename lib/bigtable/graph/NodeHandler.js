...
class NodeHandlerSingle {
    ...
    async createEdgeWithId(fnId, snId, relation = 0, attributes = {}, _extend = {}, ttld = false, depthMode = false, isPopularRightNode = false) {
        ...
        if (this.dbConfig.rightNodeEdge) {
           ...
           //TODO: if popularRightEdge mode treat differently (currently just dont write it to db)
            ...
            // Save popularnode edge data in separate table
                ...
                const qualifier = `${this.columnFamilyPopnode.familyName}:${column}`;
                requests.push(depthMode ? row.increment(qualifier) : row.save(qualifier,val));
