...
class GraphAccess {
    ...
    async upsertSingleEdgeRelationBetweenNodes(body, withTransaction = true){
        ...
        if (!depthBeforeCreation) {
            const edgeId = await this.nodeHandler.createEdgeWithId(presumedNodeLeft, presumedNodeRight, relation, edgeData, {}, ttld);
            ...
            const edgeLookupKey = presumedNodeLeft + "_" + presumedNodeRight + "_" + strToInt(relation);
            const edgeIdentifier = strToInt(edgeLookupKey) + "";
            edge = await this.nodeHandler.getEdgeByIdentifier(edgeIdentifier);
            ...
            if (!edge) {
                ...
                await this.nodeHandler.increaseEdgeDepthByEdgeId(edgeIdentifier);
            }
