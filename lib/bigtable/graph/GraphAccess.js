...
class GraphAccessSingle {
    ...
    async upsertSingleEdgeRelationBetweenNodes(body, withTransaction = true){
        ...
        const nodeLeftExists = await this.nodeHandler.getNodeByIdentifier(presumedNodeLeft);
        const nodeRightExists = await this.nodeHandler.getNodeByIdentifier(presumedNodeRight);
        ...
        if (!depthBeforeCreation) {
            const edgeId = await this.nodeHandler.createEdgeWithId(presumedNodeLeft, presumedNodeRight, relation, edgeData, {}, ttld, depthBeforeCreation, isPopularRightNode);
            edge = {
                edgeId,
            ...
            if (!edges) {
                edgeId = await this.nodeHandler.createEdgeWithId(presumedNodeLeft, presumedNodeRight, relation, edgeData, {}, ttld, depthBeforeCreation, isPopularRightNode);
