        ...
        const resultsPromise = nodes.map(
            ...
            async node => {
                ...
                for(let i = 0; i < Object.keys(node).length; i++) {
                    ...
                    if( Object.keys(node)[i].includes("ec") ||  Object.keys(node)[i].includes("ed")) {

                        const rightNodeId =  Object.keys(node)[i].split("#")[1];
                        const relation =  Object.keys(node)[i].split("#")[2];

                        const edgeObj = {
                            leftNodeId: node.identifier,
                            rightNodeId,
                            data: node[ Object.keys(node)[i]],
                            edgeTime: await this.nodeHandler.getEdgeTime(node.identifier, rightNodeId, relation)
                        };
                        ...
                        rawOtherNodeIdentifiers.push( Object.keys(node)[i].split("#")[1]);
                    } 
