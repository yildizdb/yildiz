...
class GraphAccessSingle {
    ...
    async upsertSingleEdgeRelationBetweenNodes(body, withTransaction = true){
        ...
        let nodeLeft = await this.nodeHandler.getNodeByIdentifier(presumedNodeLeft);
        let nodeRight = await this.nodeHandler.getNodeByIdentifier(presumedNodeRight);
        
        // If the nodes don't exist create them
        if (!nodeLeft) {
            nodeLeft = await this.nodeHandler.createNode(presumedNodeLeft, leftNodeData, {}, ttld, leftNodeIdentifierVal);
        }

        if (!nodeRight) {
            nodeRight = await this.nodeHandler.createNode(presumedNodeRight, rightNodeData, {}, ttld, rightNodeIdentifierVal);
        }
