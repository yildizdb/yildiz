...
class Metadata {
    ...
    decreaseCount(table, amount = 1){
      ...
      // Chris: decreases only work if they are diretly applied to the big table 
      // database, or if they go negative and the interval will subtract them
    ...
    async save() {
      ...
      // Chris: yep, save every x seconds as increase to database (if not present, 
      // increase "createRule" will create the table so this should be easy)
      // also the in memory counts have to be reset to 0, when they are increased in the bigtable metadata table
