...
class Metadata {
    ...
    async _init() {
      ...
      const {saveMetadataInSec, readMetadataInSec} = this.config;
      ...
      debug(`running interval to read metadata every ${readMetadataInSec || DEFAULT_COUNTS_INTERVAL_SEC} seconds`);
      this._runIntervalCounts(readMetadataInSec || DEFAULT_COUNTS_INTERVAL_SEC);
    }
