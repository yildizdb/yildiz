    ...
    _deleteTable() {
        ...
        const remove = (type) => {
            ...
            return async (keys) => {
                ...
                if (type === "edge") {
                    ...
                    deletedCounts = await Promise.all(
                        cleanedKeys.map(key => {
                            ...
                            const anotherNodeKey = columnKey.split("#")[1];
                            const relation = columnKey.split("#")[2];

                            this.yildiz.edgeLifetime.removeEdgeTTLFireAndForget(nodeKey, anotherNodeKey, relation);
                ...
                // If it is node, remove the ttl in redis for edges
                if (type === "node") {
                    cleanedKeys.map(key => {
                        this.yildiz.edgeLifetime.removeEdgeTTLFromNodeFireAndForget(key);
                    });
                }
