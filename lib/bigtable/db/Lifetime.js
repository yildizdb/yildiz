...
class Lifetime {
    ...
    _deleteTable() {
        ...
        const remove = (type) => {
            ...
            return async (keys) => {
                ...
                // const deletedCounts = await new Promise((resolve, reject) => {
                //     const arr = [];
                //     eachLimit(
                //         cleanedKeys,
                //         10000,
                //         (key, next) => {
                //             try {
                //                 this.yildiz[tableName].row(key).delete();
                //             }
                //             catch(err) {
                //                 // DO Nothing
                //             }
    ...
    async _job(){
        ...
        // Remove Translates
        const translateKeys = await this._getTTLIds("translates");
        results.push(await deleteTTLOrigin.translate(translateKeys));
        ...
        // Remove TTL 
        const ttlKeys = translateKeys.concat(edgeKeys).concat(nodeKeys);
