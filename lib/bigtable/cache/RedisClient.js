...
class RedisClient {
  ...
  setLastAccess(keys) {
    ...
    return this.redis.zadd(...params);
  }

  setCacheRefresh(keys) {
  ...
  getLastAccess(lastAccess, limit) {
    ...
    return this.redis
      .zrangebyscore(LASTACCESS_SET, lastAccess, Date.now(), "LIMIT", 0, limit);
  }

  async getCacheRefresh(lastRefresh, limit) {
