...
class DepthWalker {
    ...
    async _walk(){
        ...
        console.log(startT, edgeIds);
        console.log(await Promise.all(edgeIds.map(async edgeId => {
            try {
                return await this.depthTransfer.call(edgeId);
            } catch(error){
                console.log("execution error of depth transfer:", error.message, "for edge:", edgeId);
                return null;
            }
        })));
