...
class RelationUpsert extends Procedure {

    constructor(yildiz){
        super("y_relation_upsert", yildiz);
    }

    async call(identifier1, identifier2, data1, data2, value1, value2, ttld, relation, data3, mode){
        const upsertNodes = `CALL ${super.getName()}(123125, 123126, '{}', '{}', 'bla', 'blup', 0, '1', '{}', 0);`;
        await this.yildiz.raw(upsertNodes, {}).then(result => {

            const [results, _] = result;
            const {id1, id2} = results["0"]
    ...
    async storeProcedure(force = false){
        ...
        const procedure = `CREATE PROCEDURE ${super.getName()}
            ...
            BEGIN
                ...
                BEGIN
                    ROLLBACK;
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Unknown error during procedure', MYSQL_ERRNO = 1001;
                END;
