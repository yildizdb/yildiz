...
class YildizFactory {
    ...
    constructor(config = {}) {
        ...
        if(!config.access){
            ...
            if(config.access === "*"){
                ...
                debug("access is configured to ", config.access, " which is not supported, defaulting to * - allowing any prefix.");
                config.access = "*"; //TODO exit error
            }
            ...
            if(config[*]){
                ...
                debug("access is allowed for these prefixes only:", Object.keys(config).join(", "));
            }
        }
    ...
    isPrefixWithTokenAllowed(prefix, token){
        ...
        if(this.access[prefix]){
            ...
            //TODO add array or string check during startup, also check that key * must be array

            if(Array.isArray(this.access[prefix])){
                //TODO contains check with token
            } else {
                //should be string = *
                return true;
            }
