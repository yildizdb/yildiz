...
module.exports = (instance, options, next) => {
    ...
    instance.post("/upsert-singular-relation", SCHEMES.SC_RELATION, async (req, res) => {
        ...
        return await graphAccess.upsertSingleEdgeRelationBetweenNodes(req.body, false);
    });

    instance.post("/upsert-singular-relation-transaction", SCHEMES.SC_RELATION, async (req, res) => {

        const prefix = getPrefixHeader(req);
        const yildiz = await instance.factory.get(prefix);
        const graphAccess = await yildiz.getGraphAccess();

        return await graphAccess.upsertSingleEdgeRelationBetweenNodes(req.body, true);
    });
