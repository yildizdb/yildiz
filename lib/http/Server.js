...
const {YildizFactory: RDBMSFactory} = require("./../rdbms/YildizFactory.js");
const {YildizFactory: BTFactory} = require("./../bigtable/YildizFactory.js");
...
class Server {
    ...
    constructor(port = 3058, options = {}) {
        ...
        if(!options.database){
            debug("database configuration is missing.");
            options.database = {};
        }

        if (options.database.dialect === "bigtable") {
            this.factory = new BTFactory(options);
        }
        else {
            this.factory = new RDBMSFactory(options);
        }
    ...
    _init() {
        ...
        this.app.addHook("onResponse", (res, next) => {
            ...
            if(res.statusCode !== 500 && res.statusCode !== 400){
                this.factory.metrics.set("http_avg_res_time", diff);
            }

            next();
        });

        /* use the following when going for fastify@0.44.0
        this.app.setNotFoundHandler((request, res) => {

            this.incStat("not_found");
            debug(`Not found - 404 -  ${res._url}.`);
        ...
        this.app.setErrorHandler((error, request, res) => {
    ...
    listen() {
        return new Promise((resolve, reject) => {
            this.app.listen(this.port, INTERFACE, error => {
