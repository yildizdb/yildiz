...
class Server {
    ...
    _init() {
        ...
        this.app.use((req, res, next) => {
            ...
            if(!this.factory.isPrefixWithTokenAllowed(req.headers["x-yildiz-prefix"], req.headers["authorization"])){
                this.incStat("auth_failed");
                res.statusCode = 401; //TODO return other code if token is missing (there should be different for when prefix is bad)
                return res.end(JSON.stringify({
                    error: "TODO"
                }));
            }
        ...
        this.app.addHook("onResponse", (res, next) => {
            ...
            if(res.statusCode !== 500 && res.statusCode !== 400 && res._inc){
                const diff = Date.now() - res._inc;
