...
class NodeHandler {
    ...
    async edgeExistsId(firstId, secondId, relation = "unknown"){
        ...
        const firstNode = `${this.krakn.prefix}_node_id`;
        const secondNode = "other_node_id";
        const query = `SELECT depth, relation, data
            FROM ${table} 
    ...
    async createEdgeWithId(fnId, snId, relation = "unknown", attributes = {}, _extend = {}){
        
        if(!fnId || !snId){
            throw new Error("missing left or right id params.");
        }

        const leftNode = await this.getNodeByPropertyField("id", fnId);
        const rightNode = await this.getNodeByPropertyField("id", snId);

        if(!leftNode){
            throw new Error(`leftNode for id ${fnId} does not exist.`);
        }

        if(!rightNode){
            throw new Error(`rightNode for id ${snId} does not exist.`);
        }

        return this.createEdge(leftNode, rightNode, relation, attributes, _extend);
    }
